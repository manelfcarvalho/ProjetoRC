/* ==============================================================
   ...
   ============================================================== */
#define _POSIX_C_SOURCE 200809L   /* expõe nanosleep() */
#include "powerudp.h"
#include <stdio.h>
#include <stdlib.h>
#include <string.h>
...
/* ----- loop de retransmissão (back-off) ----- */
...
int receive_message(void *buf, int buflen) {
    ...
    if (h->flags & PUDP_F_CFG) {
        if ((size_t)n >= sizeof(PUDPHeader) + sizeof(ConfigMessage)) {
            apply_config((ConfigMessage*)(frame + sizeof(PUDPHeader)));
        }
        return 0;
    }
